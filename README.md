...
9. 「F7開始までの待ち時間」を3～30秒で指定します（初期値8秒）。
10. Kindleを全画面へ戻し、マウスを動かさず `F7` を押します。指定秒数後、上下の操作バーが消えてから開始します。
11. 全画面のまま `F8` で一時停止／再開、`F9` を2回で終了してPDF作成できます。
12. スクリーンショット終了直後に音と「画面を動かしてOK」の通知が出ます。その後PDF・OCRはバックグラウンドで続きます。
